...
// Types
/** Owner contract for values a table cell can render or sort: JSON scalars only. */
export type NexGTableCellValue = string | number | boolean | null | undefined;

/** Row view for accessor lookup: the caller's `data: T[]` objects keyed by column accessor. */
export type NexGTableRowRecord = {
  [key: string]: NexGTableCellValue | undefined;
};

/**
 * Single I/O-boundary reader for accessor lookup: every filter/sort/cell path funnels
 * through here so the row-object indexing assertion lives in exactly one place.
 */
function readCellValue<T>(row: T, accessorKey: string): NexGTableCellValue {
  // SAFETY: rows are the caller's own `data: T[]` data objects and accessorKey comes from
  // the caller-supplied column defs for the same T; the result is re-validated downstream
  // (null-guards, text guard, String() coercion) before render or compare.
  const record = row as NexGTableRowRecord;
  return record[accessorKey];
}

/**
 * Domain guard: table text cells sort with locale rules while other scalars rank.
 * Reads as a domain predicate (text vs scalar) so sort/render branch on the parsed
 * value instead of narrowing representations inline.
 */
function isTextValue(value: NexGTableCellValue | React.ReactNode): value is string {
  return Object.prototype.toString.call(value) === '[object String]';
}

/**
 * Rank two non-empty scalar cell values the way the table has always ordered them:
 * text pairs with locale rules, scalar pairs numerically, mixed pairs as text.
 */
function compareCellValues(a: string | number | boolean, b: string | number | boolean): number {
  if (isTextValue(a) && isTextValue(b)) return a.localeCompare(b);
  if (!isTextValue(a) && !isTextValue(b)) {
    const left = Number(a);
...
export interface NexGTableColumn<T = unknown> {
  ...
  width?: number | string;
  minWidth?: number;
  cell?: (value: NexGTableCellValue, row: T) => React.ReactNode;
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    if (searchQuery && filterable) {
      processedData = processedData.filter((row) =>
        columns.some((column) => {
          ...
          const value = readCellValue(row, column.accessorKey);
          return String(value || '')
            .toLowerCase()
    ...
    if (sortState.column && sortState.direction && sortable) {
      ...
      processedData.sort((a, b) => {
        const aValue = readCellValue(a, sortColumn);
        const bValue = readCellValue(b, sortColumn);
        ...
        style={[
          {
            ...
            // SAFETY: width is typed number|string in the NexGTableColumn owner contract
            // (checked by tsc at call sites); RN resolves numbers and "N%" strings.
