...
import { OPS_BASE, getOpsToken } from '@/lib/api';
...
/** Ops thread view — same shared system, full history, live poll. */
export default function ThreadScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const client = createMessagingClient({ base: OPS_BASE, getToken: () => getOpsToken() });
  ...
  return (
    <View style={{ flex: 1, paddingTop: insets.top }}>
      <NexGMessageThread
        messages={messages}
        myAccountId={accountIdFromToken(getOpsToken())}
