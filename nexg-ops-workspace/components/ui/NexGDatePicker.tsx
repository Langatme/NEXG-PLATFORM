...
// Type guard to check if value is NexGDateRange
const isDateRange = (
...
export function NexGDatePicker(props: NexGDatePickerProps) {
  ...
  const handleConfirm = () => {
    feedback("success");
    if (mode === "range") {
      (onChange as (value: NexGDateRange | undefined) => void)?.(tempRange);
    } else {
  ...
  const clearSelection = () => {
    if (mode === "range") {
      ...
      (onChange as (value: NexGDateRange | undefined) => void)?.(undefined);
    } else {
